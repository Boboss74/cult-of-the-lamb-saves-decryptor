class CotLSaveFile {
  ...
  constructor(file: File, fileBin: Uint8Array) {
  ...
  static async create(file: File): Promise<CotLSaveFile> {
  ...
  async getNewContent(): Promise<Uint8Array> {
    if (this.isCrypted()) {
...
class CryptoHelper {
   ...
   */
  static async decrypt(data: Uint8Array, encryptionKey: Uint8Array, iv: Uint8Array): Promise<Uint8Array> {
   ...
   * @param iv - The initialization vector.
   */
  static async encrypt(data: Uint8Array, encryptionKey: Uint8Array, iv: Uint8Array): Promise<Uint8Array> {
    // Import the encryption key
   ...
   */
  static binToHexString(buffer: Uint8Array): string {
   ...
   */
  static hexStringToBin(hexString: string): Uint8Array {
    if (hexString.length % 2 !== 0) {
      throw new Error('Invalid hex string.');
    }

    const byteArray = Array.from({ length: hexString.length / 2 }, (_, idx) =>
      parseInt(hexString.substr(idx * 2, 2), 16)
    );
...
class FileHelper {
  static async fileToBin(file: File): Promise<Uint8Array> {
    return new Promise<Uint8Array>((resolve, reject) => {
      const fileReader = new FileReader();

      fileReader.onload = (event) => {
        resolve(new Uint8Array(event.target?.result as ArrayBuffer)); // TODO
      };
   ...
   */
  static async triggerFileDownload(content: Uint8Array, fileName: string): Promise<void> {
